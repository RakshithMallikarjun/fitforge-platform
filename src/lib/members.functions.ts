...
export const getMember = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: user }, { data: profile }, { data: assigns }, { data: assessments }, { data: plans }, { data: attendance }] = await Promise.all([
      ...
      supabase.from("attendance_logs").select("id, member_id, gym_id, check_in_at, check_out_at, location_type, created_at").eq("member_id", data.memberId).order("check_in_at", { ascending: false }).limit(100),
    ]);
